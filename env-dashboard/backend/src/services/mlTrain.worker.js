...
const fs = require('fs');
const { loadDataset, loadMetadata, trainTestSplit, FEATURES } = require('./mlDataset');
...
function train(dataset, options = {}) {
  ...
  if (verbose) {
    console.log('[ml-train-worker] Final training metrics:');
    console.log(`[ml-train-worker]   accuracy=${trainMetrics.accuracy.toFixed(4)}`);
    console.log(`[ml-train-worker]   precision=${trainMetrics.precision.toFixed(4)}`);
    console.log(`[ml-train-worker]   recall=${trainMetrics.recall.toFixed(4)}`);
    console.log(`[ml-train-worker]   f1=${trainMetrics.f1.toFixed(4)}`);
    console.log(`[ml-train-worker]   auc=${auc.toFixed(4)}`);
  }
...
function saveModel(model, metadata) {
  ...
  console.log(`[ml-train-worker] Model saved to ${MODEL_FILE}`);
  return MODEL_FILE;
}

function saveModelInfo(modelInfo) {
  fs.writeFileSync(MODEL_INFO_FILE, JSON.stringify(modelInfo, null, 2));
}

parentPort.on('message', async (message) => {
  const { type, id, requestedBy } = message;

  if (type === 'train') {
    try {
      console.log('[ml-train-worker] Starting training job:', id);

      const dataset = loadDataset();
      if (!dataset || dataset.length === 0) {
        throw new Error('No dataset found. Run buildDataset first.');
      }

      const { train: trainSet, test: testSet } = trainTestSplit(dataset, 0.2, 42);
      console.log(`[ml-train-worker] Training set: ${trainSet.length}, Test set: ${testSet.length}`);

      const metadata = loadMetadata();
      if (!metadata) {
        throw new Error('No dataset metadata found.');
      }

      const model = train(trainSet, { learningRate: 0.1, iterations: 1000, verbose: true });

      const testMetrics = evaluate(model, testSet);
      console.log(`[ml-train-worker] Test accuracy: ${testMetrics.accuracy}`);
      console.log(`[ml-train-worker] Test F1: ${testMetrics.f1}`);
      console.log(`[ml-train-worker] Test AUC: ${testMetrics.auc}`);

      saveModel(model, metadata);

      const evalStart = Date.now();
      for (const r of testSet) {
        inference(r.raw || r, model, metadata);
      }
      const evalMs = Date.now() - evalStart;
      const latency = {
        evalMs,
        perSampleMs: +(evalMs / Math.max(1, testSet.length)).toFixed(3),
        samplesPerSecond: +(testSet.length / Math.max(0.001, evalMs / 1000)).toFixed(2),
      };
      console.log(`[ml-train-worker] Inference latency: ${evalMs}ms`);

      const completedAt = new Date().toISOString();
      const modelInfo = {
        modelType: 'LOGISTIC_REGRESSION',
        modelVersion: 'lr-1.0.0',
        trainedAt: completedAt,
        trainingTimestamp: completedAt,
      ...
      const result = {
        ...
        roc: { points: testMetrics.roc, auc: testMetrics.auc },
        featureImportance: model.featureImportance || [],
        latency,
        threshold: message.threshold,
        notes: `Trained logistic regression model. Dataset: ${dataset.length} records. Train/Test split: ${trainSet.length}/${testSet.length}.`,
        datasetInfo: {
          recordCount: dataset.length,
      ...
      console.log('[ml-train-worker] Training complete successfully');
      parentPort.postMessage({ type: 'result', id, result });
    } catch (error) {
      console.error('[ml-train-worker] Training failed:', error);
